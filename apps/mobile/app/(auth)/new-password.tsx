...
export default function NewPassword() {
  ...
  const strength = password.length >= 12 ? 'strong' : password.length >= 8 ? 'medium' : password.length > 0 ? 'weak' : 'empty'

  return (
    <SafeAreaView className="flex-1 bg-cream-50">
      <Stack.Screen options={{ title: 'Yeni Şifre', headerStyle: { backgroundColor: '#FBF8F0' } }} />
      <View className="flex-1 px-6 pt-8">
        ...
        <View className="gap-3">
          <Input label="YENİ ŞİFRE" value={password} onChangeText={setPassword} placeholder="•••••••••••" secureTextEntry />

          {strength !== 'empty' && (
            <View className="flex-row gap-1">
              <View className={`flex-1 h-1 rounded-full ${strength === 'weak' ? 'bg-red-400' : strength === 'medium' ? 'bg-amber-400' : 'bg-green-500'}`} />
              <View className={`flex-1 h-1 rounded-full ${strength === 'medium' ? 'bg-amber-400' : strength === 'strong' ? 'bg-green-500' : 'bg-slate-200'}`} />
              <View className={`flex-1 h-1 rounded-full ${strength === 'strong' ? 'bg-green-500' : 'bg-slate-200'}`} />
            </View>
          )}

          <Input label="TEKRAR" value={confirm} onChangeText={setConfirm} placeholder="•••••••••••" secureTextEntry />
