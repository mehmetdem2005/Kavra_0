...
export async function engineRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/engine/outcome', async (req, reply) => {
    ...
    if (!lesson.technique_id) {
      return reply.code(400).send({ error: 'no_technique', message: 'Ders henüz tekniğe bağlanmamış' })
    }
    ...
    const duration = Math.floor(
      (Date.now() - new Date(lesson.started_at).getTime()) / 1000,
    )
