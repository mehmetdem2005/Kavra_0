...
export async function sharingRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { slug: string } }>(
    ...
    async (req, reply) => {
      ...
      const { data: sources } = await supabase
        ...
        .select('id, title, source_type, status, char_count, youtube_duration_seconds, transcript_source')
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      if (!ent?.is_pro) {
        return reply.code(402).send({ error: 'pro_required', message: 'Defter klonlama Pro özelliğidir.' })
      }
